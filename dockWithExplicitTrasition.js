...
var Dock = GObject.registerClass(
	class Dock extends St.Bin {
		...
		_activate() {
			this._showHideTransition(true);
			//			this.set_height(-1);
		}

		_deactivate() {
			this._showHideTransition(false);
			//			this.set_height(1);
		}

		_showHideTransition(show) {
			let direction = show ?
				Clutter.TimelineDirection.FORWARD :
				Clutter.TimelineDirection.BACKWARD;
			let showHide = this.get_transition('showHide');
			if (showHide) {
				_log(`change direction: ${show ? 'show' : 'hide'}`);
				showHide.set_direction(direction);
			} else {
				let height = this._getNaturalHeight();
				_log(`height for transition: ${height}`);
				showHide = new Clutter.PropertyTransition({
					property_name: 'height',
					duration: 200,
					remove_on_complete: true
				});
				showHide.set_from(1);
				showHide.set_to(height);
				showHide.set_direction(direction);
				this.add_transition('showHide', showHide);
			}
		}
